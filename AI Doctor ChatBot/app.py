                ...
                if message.get('sources'):
                    with st.expander("📚 View Sources", expanded=False):
                        ...
                        for idx, source in enumerate(message['sources']):
                            col = cols[idx % 2]
                            with col:
                                # Extract metadata and content based on source type
                                if hasattr(source, 'metadata'):  # Document object
                                    doc_source = getattr(source.metadata, 'source', 'Unknown')
                                    page = getattr(source.metadata, 'page', 'N/A')
                                    content = source.page_content if hasattr(source, 'page_content') else str(source)[:300]
                                elif isinstance(source, dict):  # Dictionary
                                    doc_source = source.get('metadata', {}).get('source', 'Unknown')
                                    page = source.get('metadata', {}).get('page', 'N/A')
                                    content = source.get('page_content', str(source))[:300]
                                else:  # Fallback
                                    doc_source = 'Unknown'
                                    page = 'N/A'
                                ...
                                st.markdown(f"""
                                <div style='background-color: #f0f2f6; padding: 1rem; border-radius: 8px; margin: 0.5rem 0;'>
                                    <p><strong>Source {idx + 1}</strong></p>
                                    <p><code>{doc_source}</code></p>
                                    <p>Page: {page}</p>
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        st.session_state.index_mode = "existing" if index_mode == "Use existing index" else "new"
        ...
        query_type = st.selectbox(
            "Query Type",
            ["medical", "educational", "detailed", "general"],
            index=0
        )
        ...
        if st.session_state.index_mode == "existing":
            if st.button("Initialize with Existing Index", type="primary", use_container_width=True):
...
def process_documents(uploaded_files, model_name, search_k):
    ...
    try:
        ...
        with st.spinner("🔍 Processing medical documents..."):
            ...
            medbot.setup(create_new_index=True)
            medbot.doc_chat.retriever.search_kwargs["k"] = search_k
...
def main_app():
    ...
    if not st.session_state.docs_processed:
        if st.session_state.index_mode == "existing":
            ...
            st.info("👋 Please upload medical documents and click 'Process Documents' to begin")
        return
        ...
        with st.spinner("🔬 Analyzing your question..."):
            try:
                if st.session_state.medbot is None:
                    st.error("MedBot is not initialized. Please initialize with an index first.")
                    return
                ...
                # # Also add the error to chat history
                # st.session_state.chat_history.append({
                #     "role": "assistant",
